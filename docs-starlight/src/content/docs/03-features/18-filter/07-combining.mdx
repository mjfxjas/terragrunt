...
When using [Git expressions](../git), components are discovered in temporary Git worktrees (see [How it works](../git#how-it-works) for more details. As such, relative paths resolve relative to the root of the Git worktree where the component was discovered.
...
Note the use of `awk` here to wrap paths in `{}`. This is to explicitly mark the paths as [path expressions](../path). This is only necessary for disambiguation between [path expressions](../path) and [name expressions](../name). If you aren't concerned with this ambiguity, you can omit this step.
